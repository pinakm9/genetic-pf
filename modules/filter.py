...
class Filter():
    ...
    def compute_error(self, hidden_path):
        ...
        if hasattr(self, 'record_path'):
             hdf5 = tables.open_file(self.record_path, 'a')
             hdf5.create_array(hdf5.root, 'l2_error', self.rmse)
...
class ParticleFilter(Filter):
    ...
    def __init__(self, model, particle_count, folder=None,  particles = None):
        ...
        if folder is not None:
            ...
            self.hdf5 = tables.open_file(self.record_path, 'w')
            ...
            self.hdf5.create_group('/', 'particles')
            self.hdf5.create_group('/', 'weights')
            rs = self.hdf5.create_table(self.hdf5.root, 'resampling', self.bool_description)
            rs.flush()
            obs = self.hdf5.create_table(self.hdf5.root, 'observation', observation_description)
            obs.flush()
        else:
    ...
    def record(self, observation):
        ...
        if self.recording:
            ...
            self.hdf5.root.resampling.append(np.array([self.resampling_tracker[self.current_time]], dtype=np.bool_))
            self.hdf5.root.resampling.flush()
            # record observation
            self.hdf5.root.observation.append(np.array(observation, dtype = np.float64))
            self.hdf5.root.observation.flush()

    @ut.timer
    def update(self, observations, threshold_factor = 0.1, method = 'mean', resampling_method = 'systematic',\
